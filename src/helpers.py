...
def cast_tuple(val, length: int = None) -> tuple:
    '''
    Casts input to a tuple. If the input is a list, converts it to a tuple.
    If input a single value, casts it to a tuple of length `length`, which is 1 if not provided.
    '''
...
def default(val, d):
    """
    Returns the input value `val` unless it is `None`
    """
    if exists(val):
        return val
...
def eval_decorator(fn):
    """
    Decorator for sampling from Imagen.
    Temporarily sets the model in evaluation mode if it was training.
    """
